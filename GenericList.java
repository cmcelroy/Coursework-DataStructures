...
{
	private mytype data;
	private Node<mytype> link;

	// constructor
	public mytype Node(mytype data)
	{
		this.data = 0;
		this.link = null;
	}
	...
	public Node<mytype> getLink()
	{
		return this.link;
	}

	public void setLink(Node<mytype> link)
	{
		this.link = link;
	}
}
...
public class List<mytype>
{
	...
	public <mytype> void InsertBefore(mytype data)
	{
		if(!this.IsFull())
		{
			...
			{
				//if we're at the beginning, just crearte a new head that points to the current one
	...
	// the new element becomes the current
	// this should not be possible for a full list
	public <mytype> void InsertAfter(mytype data)
	{
		if (!this.IsFull())
		{
			Node n = new Node();
			...
			//if the list is empty, everything pooints to the single node
			if( this.IsEmpty())
				this.head = this.tail = this.curr = n;
			
			else
			{
				//if we're at the end, just tank the new node on
	...
	public void Remove()
	{
		if(!this.IsEmpty())
		{
			...
			{
				this.Prev();
				// and rearrange the point to remove this node
				this.curr.setLink(this.curr.getLink().getLink());
				//we handle removing the tail vs. otehr internal nodes a bit differently
	...
	// this should not be possible for an empty list
	public <mytype> void Replace(mytype data)
	{
		if(!this.IsEmpty())
	...
	{
		//list are no equal or differnet sizes
		...
		//iterate througheach list
		while (p != null)
		{
			//if any pair of elements differ, the list are not equal
